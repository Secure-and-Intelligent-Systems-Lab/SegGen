...
def fix_seeds(seed: int = 69) -> None:
...
def setup_ddp(rank, world_size):
    ...
    port = os.environ.get("MASTER_PORT", "12351")
    os.environ['MASTER_ADDR'] = addr
    os.environ['MASTER_PORT'] = port
    dist.init_process_group("nccl", rank=rank, world_size=world_size)
    torch.cuda.set_device(rank)

# def setup_ddp(rank = 0):
...
#         dist.barrier()
#     elif os.environ['WORLD_SIZE'] == '1':
#         os.environ['MASTER_ADDR'] = 'localhost'
#         os.environ['MASTER_PORT'] = '23456'
#         dist.init_process_group("gloo", init_method="env://?use_libuv=False", rank=rank, world_size=int(os.environ['WORLD_SIZE']),
...
#         dist.init_process_group("nccl",  init_method="env://?use_libuv=False", rank=rank, world_size=int(os.environ['WORLD_SIZE']), timeout=datetime.timedelta(seconds=7200))
#         torch.cuda.device(rank)
#     return
...
def get_logger(log_file_path):
    logger = logging.getLogger("FuseFormTrainer")
    logger.setLevel(logging.INFO)
...
def print_results(epoch, iou, miou, acc, macc, class_names):
